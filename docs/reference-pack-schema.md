...
AI-ST can optionally load operator-supplied validation rules from the directory named by `REFERENCE_DIR`. Reference packs are not bundled with the application. Operators are responsible for accuracy, licensing, review, and change control.
...
When `REFERENCE_DIR` is unset, AI-ST starts with reference validation disabled. Authenticated users can inspect `GET /api/references/status`, which returns `{ enabled, filesLoaded, rulesLoaded }`. Each analysis also emits `reference_pack_unavailable` so the interface cannot imply that deterministic reference validation ran.
