    ...
    return ChannelInventory(
        ...
        channels=[
            ChannelInfo(index=0, label='Airflow', sample_rate=10.0,
                        ...
                        duration_sec=dur, n_samples=int(10.0 * dur)),
...
def _flow(start: float, end: float, *, mag: float = 0.5,
          score: float = 1.0, notes: list[str] | None = None) -> CandidateWindow:
    return CandidateWindow(
        start_sec=start, end_sec=end,
        label='provisional_flow_reduction', channel='Airflow',
        magnitude=mag, priority_score=score, notes=notes or [],
    )
...
def _desat(start: float, end: float, *, score: float = 0.8) -> CandidateWindow:
    return CandidateWindow(
        start_sec=start, end_sec=end,
        label='provisional_desaturation', channel='SpO2',
        magnitude=4.5, priority_score=score,
    )
...
def test_event_id_absent_when_event_not_in_slicer_set() -> None:
    ...
    position_event = CandidateWindow(
        start_sec=120.0, end_sec=125.0,
        label='provisional_position_change', channel='Position',
        magnitude=1.0, priority_score=0.7,
    )
    cset = CandidateSet(
        windows=[position_event],
        channels_used=['Position'], channels_missing=[],
    )
    pkg = _package(cset)
...
def test_package_fits_within_8kb_with_many_candidates() -> None:
    ...
    events = [
        _flow(float(i * 30 + 10), float(i * 30 + 25), score=1.0 - i * 0.01)
        for i in range(100)
    ]
    cset = CandidateSet(windows=events, channels_used=['Airflow'], channels_missing=['SpO2'])
    pkg = _package(cset)
...
def test_candidate_count_trimmed_from_llm_package_reflects_truncation() -> None:
    ...
    events = [
        _flow(float(i * 30 + 10), float(i * 30 + 25))
        for i in range(100)
    ]
    cset = CandidateSet(windows=events, channels_used=['Airflow'], channels_missing=['SpO2'])
    pkg = _package(cset)
