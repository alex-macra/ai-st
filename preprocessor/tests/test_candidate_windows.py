...
def test_flow_reduction_5s_detected_with_peds_params():
    ...
    windows = _detect_flow_reductions(sig, sr, min_duration_sec=6.0, threshold_pct=0.3, envelope_sec=4.0)
    assert len(windows) >= 1
...
def test_flow_reduction_5s_missed_with_adult_params():
    ...
    windows = _detect_flow_reductions(sig, sr, min_duration_sec=10.0, threshold_pct=0.3, envelope_sec=10.0)
...
def _flow(start: float, end: float, mag: float = 0.5) -> CandidateWindow:
    return CandidateWindow(
        start_sec=start, end_sec=end, label="provisional_flow_reduction", channel="Flow", magnitude=mag
    )


def _desat(start: float, end: float, mag: float = 4.0) -> CandidateWindow:
    return CandidateWindow(
        start_sec=start, end_sec=end, label="provisional_desaturation", channel="SpO2", magnitude=mag
    )
...
def test_tagged_flow_events_is_complement_of_headline() -> None:
    ...
    assert all(TAG_UNCOUPLED_HYPOPNEA not in e.notes and TAG_OVERLAPS_FLAT not in e.notes for e in headline)
    assert all(TAG_UNCOUPLED_HYPOPNEA in e.notes or TAG_OVERLAPS_FLAT in e.notes for e in tagged)
...
def test_position_artifact_tagged_within_proximity() -> None:
    ...
    out, stats = _post_process_flow_events([flow], [], flat_intervals=[], position_transition_secs=[100.0])
...
def test_position_artifact_no_transitions_no_tag_no_error() -> None:
    ...
    flow = _flow_w(50, 65)
    out, stats = _post_process_flow_events([flow], [], flat_intervals=[], position_transition_secs=None)
    assert TAG_POSITION_ARTIFACT not in out[0].notes
    assert stats["tagged_position_artifact"] == 0
