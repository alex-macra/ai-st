...
from candidate_windows import CandidateSet, CandidateWindow, TAG_UNCOUPLED_HYPOPNEA, assign_event_ids
...
def _airflow_header(*, sample_frequency: int = 10) -> dict:
    return {
        'label': 'Airflow', 'dimension': 'mV', 'sample_frequency': sample_frequency,
        'physical_min': -2.0, 'physical_max': 2.0,
        'digital_min': -32768, 'digital_max': 32767, 'prefilter': '', 'transducer': '',
    }


def _spo2_header(*, sample_frequency: int = 1, physical_min: float = 50.0) -> dict:
    return {
        'label': 'SpO2', 'dimension': '%', 'sample_frequency': sample_frequency,
        'physical_min': physical_min, 'physical_max': 100.0,
        'digital_min': -32768, 'digital_max': 32767, 'prefilter': '', 'transducer': '',
    }
...
def _inventory(dur: float, *, flow_sr: float = 10.0, spo2_sr: float = 1.0) -> ChannelInventory:
    return ChannelInventory(
        duration_sec=dur,
        channels=[
            ChannelInfo(index=0, label='Airflow', sample_rate=flow_sr,
                        physical_min=-2.0, physical_max=2.0, unit='mV',
                        duration_sec=dur, n_samples=int(flow_sr * dur)),
            ChannelInfo(index=1, label='SpO2', sample_rate=spo2_sr,
                        physical_min=50.0, physical_max=100.0, unit='%',
                        duration_sec=dur, n_samples=int(spo2_sr * dur)),
        ]
    )
...
def _flow_candidate(start: float = 40.0, end: float = 55.0) -> CandidateSet:
    windows = [
        CandidateWindow(start_sec=start, end_sec=end,
                        label='provisional_flow_reduction',
                        channel='Airflow', magnitude=0.65,
                        priority_score=1.0)
    ]
...
def test_build_signal_slices_writes_valid_json(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_edf(edf,
               [_airflow_header(), _spo2_header()],
               [np.sin(2 * np.pi * 0.2 * t), np.full(int(dur), 95.5)])
...
def test_build_signal_slices_includes_spo2_for_flow_event(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_edf(edf,
               [_airflow_header(), _spo2_header()],
               [np.sin(2 * np.pi * 0.2 * t), np.full(int(dur), 96.0)])
...
def test_build_signal_slices_empty_candidates_writes_empty_array(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_edf(edf,
               [_airflow_header(), _spo2_header()],
               [np.sin(2 * np.pi * 0.2 * t), np.full(int(dur), 95.0)])
...
def test_build_signal_slices_spo2_dropout_serialises_as_null(tmp_path: Path, monkeypatch) -> None:
    ...
    inv = ChannelInventory(
        duration_sec=dur,
        channels=[
            ChannelInfo(index=0, label='Airflow', sample_rate=10.0,
                        physical_min=-2.0, physical_max=2.0, unit='mV',
                        duration_sec=dur, n_samples=n_flow),
            ChannelInfo(index=1, label='SpO2', sample_rate=1.0,
                        physical_min=-10.0, physical_max=100.0, unit='%',
                        duration_sec=dur, n_samples=n_spo2),
        ]
    )
    ...
    assert all(v is None for v in spo2_slice['samples']), \
        'Expected all out-of-range SpO2 values to be null'
...
def test_build_signal_slices_window_clamped_at_recording_end(tmp_path: Path, monkeypatch) -> None:
    ...
    late_candidate = CandidateSet(
        windows=[CandidateWindow(start_sec=110.0, end_sec=118.0,
                                 label='provisional_flow_reduction',
                                 channel='Airflow', magnitude=0.5,
                                 priority_score=1.0)],
        channels_used=['Airflow'], channels_missing=[],
    )
...
def test_build_signal_slices_headline_before_tagged_before_desat(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_edf(edf,
               [_airflow_header(), _spo2_header()],
               [np.sin(2 * np.pi * 0.1 * t), np.full(int(dur), 96.0)])

    tagged_event = CandidateWindow(
        start_sec=60.0, end_sec=75.0,
        label='provisional_flow_reduction', channel='Airflow', magnitude=0.4,
        priority_score=0.9, notes=[TAG_UNCOUPLED_HYPOPNEA],
    )
    headline_event = CandidateWindow(
        start_sec=110.0, end_sec=125.0,
        label='provisional_flow_reduction', channel='Airflow', magnitude=0.6,
        priority_score=0.5,  # lower priority than tagged, but still headline → ev_000
    )
    desat_event = CandidateWindow(
        start_sec=200.0, end_sec=210.0,
        label='provisional_desaturation', channel='SpO2', magnitude=5.0,
        priority_score=0.8,
    )
...
def test_build_signal_slices_missing_channel_emits_empty_slices_with_dense_ids(
    ...
    pos_header = {
        'label': 'Position', 'dimension': 'code', 'sample_frequency': 1,
        'physical_min': 0.0, 'physical_max': 8.0,
        'digital_min': -32768, 'digital_max': 32767, 'prefilter': '', 'transducer': '',
    }
    ...
    inv = ChannelInventory(
        duration_sec=dur,
        channels=[
            ChannelInfo(index=0, label='Position', sample_rate=1.0,
                        physical_min=0.0, physical_max=8.0, unit='code',
                        duration_sec=dur, n_samples=int(dur)),
        ]
    )
    windows = [
        CandidateWindow(start_sec=20.0, end_sec=35.0,
                        label='provisional_flow_reduction', channel='Airflow',
                        magnitude=0.5, priority_score=1.0),
        CandidateWindow(start_sec=60.0, end_sec=75.0,
                        label='provisional_flow_reduction', channel='Airflow',
                        magnitude=0.6, priority_score=0.8),
    ]
    assign_event_ids(windows)
...
def test_build_signal_slices_no_cap_on_event_count(tmp_path: Path, monkeypatch) -> None:
    ...
    _write_edf(edf,
               [_airflow_header(), _spo2_header()],
               [np.sin(2 * np.pi * 0.05 * t), np.full(int(dur), 94.0)])

    windows = [
        CandidateWindow(
            start_sec=float(i * 100 + 20), end_sec=float(i * 100 + 35),
            label='provisional_flow_reduction', channel='Airflow', magnitude=0.5,
