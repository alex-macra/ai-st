...
_CHANNEL_GROUPS = {
    ...
    "airflow": {"labels": {"flow", "nasal flow", "airflow", "oronasal", "ptaf", "nasal pressure"}, "color": "#60a5fa"},
...
def render_window(
    ...
    gs  = gridspec.GridSpec(
        ...
        hspace=0.0, left=0.0, right=1.0, top=1.0, bottom=0.0
    )
    ...
    buf = io.BytesIO()
    fig.savefig(buf, format="jpeg", dpi=100, facecolor=_BG_COLOR,
                pil_kwargs={"quality": 55, "optimize": True})
    buf.seek(0)
    data = buf.getvalue()
    ...
    if len(data) > 35_000:
        buf2 = io.BytesIO()
        fig.savefig(buf2, format="jpeg", dpi=100, facecolor=_BG_COLOR,
                    pil_kwargs={"quality": 30, "optimize": True})
        data = buf2.getvalue()
