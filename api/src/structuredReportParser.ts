...
export const structuredReportTolerantSchema = z
  .object({
...
export function parsePass2Output(raw: string): ParseResult | ParseFailure {
  ...
  if (typeof candidate?.summary !== 'string')
    warnings.push(`summary was ${typeof candidate?.summary}, coerced to string`);
  if (typeof candidate?.impression !== 'string')
    warnings.push(`impression was ${typeof candidate?.impression}, coerced to string`);
