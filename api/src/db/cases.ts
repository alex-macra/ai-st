  ...
  try {
    ...
    return {
      ...
      ...(rcd ? { reiCalculationDetail: {
        ...
        ...(rcd['flow_channel_flat_pct'] != null ? { flowChannelFlatPct: n(rcd, 'flow_channel_flat_pct') } : {}),
      } } : {}),
      ...(fs ? { flowStats: {
        count:                 n(fs, 'count') ?? 0,
        ...
        ...(fs['avg_duration_sec'] != null ? { avgDurationSec: n(fs, 'avg_duration_sec') } : {}),
        ...(fs['max_duration_sec'] != null ? { maxDurationSec: n(fs, 'max_duration_sec') } : {}),
        ...(fs['severity_breakdown'] != null ? { severityBreakdown: fs['severity_breakdown'] as Record<string, number> } : {}),
      } } : {}),
      ...(sp ? { spo2: {
        ...
        ...(sp['avg_desat_depth_pct']   != null ? { avgDesatDepthPct:    n(sp, 'avg_desat_depth_pct') } : {}),
        ...(sp['deepest_desat_pct']     != null ? { deepestDesatPct:     n(sp, 'deepest_desat_pct') } : {}),
        ...(sp['avg_desat_duration_sec']!= null ? { avgDesatDurationSec: n(sp, 'avg_desat_duration_sec') } : {}),
        ...(sp['longest_desat_sec']     != null ? { longestDesatSec:     n(sp, 'longest_desat_sec') } : {}),
        ...(sp['sum_desat_sec']         != null ? { sumDesatSec:         n(sp, 'sum_desat_sec') } : {}),
        ...(sp['severity_breakdown']    != null ? { severityBreakdown:   sp['severity_breakdown'] as Record<string, number> } : {}),
      } } : {}),
      ...(hr ? { hr: {
      ...
      ...(sn ? { snore: {
        ...
        ...(sn['snore_index_per_hour'] != null ? { snoreIndexPerHour:  n(sn, 'snore_index_per_hour') } : {}),
      } } : {}),
      ...(pos ? { positional: {
        ...(pos['supine_time_pct']           != null ? { supineTimePct:           n(pos, 'supine_time_pct') } : {}),
        ...
        ...(pos['upright_time_pct']          != null ? { uprightTimePct:          n(pos, 'upright_time_pct') } : {}),
        ...(pos['supine_flow_event_count']   != null ? { supineFlowEventCount:    n(pos, 'supine_flow_event_count') } : {}),
        ...(pos['nonsupine_flow_event_count']!= null ? { nonsupineFlowEventCount: n(pos, 'nonsupine_flow_event_count') } : {}),
        ...(pos['supine_rei_per_hour']       != null ? { supineReiPerHour:        n(pos, 'supine_rei_per_hour') } : {}),
        ...(pos['nonsupine_rei_per_hour']    != null ? { nonsupineReiPerHour:     n(pos, 'nonsupine_rei_per_hour') } : {}),
      } } : {}),
    } as EdfMetrics;
  } catch {
    return null;
...
function extractDemographics(casePackageJson: string | null): { ageYears?: number; sex?: 'M' | 'F' | 'X' } | undefined {
...
function rowToCase(row: DbCaseRow): Case {
  ...
  return {
    ...
    ...(row.structured_report !== null ? { structuredReport: JSON.parse(row.structured_report) as StructuredReport } : {}),
    ...(row.section_reviews !== null ? { sectionReviews: JSON.parse(row.section_reviews) as SectionReviews } : {}),
    ...(row.reference_flags !== null ? { referenceFlags: JSON.parse(row.reference_flags) as ReferenceFlag[] } : {}),
    ...(row.validation_warnings !== null ? { validationWarnings: JSON.parse(row.validation_warnings) as ValidationWarning[] } : {}),
...
export function getCases(status?: string): Case[] {
  const rows = (status
    ? getDb()
        ...
        .all(status)
    : getDb().prepare('SELECT * FROM cases ORDER BY created_at DESC').all()) as DbCaseRow[];
...
export function getCasesScoped(scope: CaseScope, status?: string): Case[] {
  ...
  const rows = getDb().prepare(sql).all(...params) as DbCaseRow[];
  return rows.map(rowToCase);
}
...
export function updateCaseStatusWithAudit(
  ...
  const transaction = db.transaction((): boolean => {
    const result = db
      .prepare("UPDATE cases SET status = ?, updated_at = ? WHERE id = ? AND status != 'signed_off'")
...
export function signOffCaseWithAudit(id: string, updatedAt: string, audit: AuditRecord): boolean {
  ...
  const transaction = db.transaction((): boolean => {
    const result = db
      .prepare("UPDATE cases SET status = 'signed_off', updated_at = ? WHERE id = ? AND status != 'signed_off'")
...
export function deleteAllCases(): number {
  ...
  const tx = db.transaction((): number => {
    db.prepare("DELETE FROM audit_log WHERE case_id IN (SELECT id FROM cases WHERE status != 'signed_off')").run();
    const result = db.prepare("DELETE FROM cases WHERE status != 'signed_off'").run() as { changes: number };
