...
import { isOfflineReport, isSyntheticInput } from '../provenance';
...
function extractScreenshotMetadata(
  casePackageJson: string | undefined,
  ...
  try {
    ...
    const metadata = pkg['screenshot_metadata'] as
      Array<{ id: string; originalName: string }> | undefined;
...
export function ReviewWorkspace({ initialCase, onBack }: Props) {
  ...
  const [validationRejections, setValidationRejections] = useState<
    Array<{ quote: string; reason: string }>
  >([]);
  ...
  async function handleFindingDecision(
    findingId: string,
  ...
  async function handleSectionDecision(
  ...
  return (
    <div className="space-y-4">
      ...
      {isSyntheticInput(c) && (
        <Alert variant="warning" title="Synthetic demo study">
          This input recording was generated, not recorded from a person. It has no clinical meaning
          and must not be used for patient care.
        </Alert>
      )}

      {isOfflineReport(c) && (
        <Alert variant="warning" title="Offline-generated report">
          This report text was generated by the built-in demo model. It has no clinical meaning and
      ...
      {showRerunConfirm && (
        <div className="rounded-lg border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 p-3 flex items-center justify-between gap-3">
          <p className="text-sm text-amber-800 dark:text-amber-300">
            Re-running will overwrite {c.findings.length} existing finding
            {c.findings.length !== 1 ? 's' : ''}. Continue?
          </p>
          ...
          {c.structuredReport ? (
            ...
            <div className="card p-4 space-y-1.5">
              ...
              <p className="text-sm leading-relaxed whitespace-pre-wrap">
                {stripInlineCitations(c.narrative)}
              </p>
            </div>
          ) : (
            <div className="card card-muted p-6 text-center">
              <p className="text-sm text-slate-500">
                No report yet. Run analysis to generate a structured report.
              </p>
            </div>
          )}
          <ScreenshotPanel
      ...
      {activeTab === 'findings' && (
        <div className="space-y-3">
          ...
          {c.findings.length > 0 ? (
              ...
              {(['high', 'medium', 'low'] as FindingConfidence[]).map((level) => {
                ...
                return (
                  <div key={level} className="space-y-2">
                    <p className={`text-xs font-semibold uppercase tracking-wide ${COLOR[level]}`}>
                      {LABEL[level]}
                    </p>
                    {group.map((f) => {
                  ...
                  return (
                    <details open className="group">
                      <summary className="cursor-pointer select-none text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors list-none flex items-center gap-2">
                        ...
                        Event Waveforms ({shown.length}
                        {signalSlices.length > SCREEN_WAVEFORM_CAP
                          ? ` of ${signalSlices.length}`
                          : ''}
                        )
                      </summary>
                      <div className="mt-3 space-y-6">
                        ...
                        {signalSlices.length > SCREEN_WAVEFORM_CAP && (
                          <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-2">
                            {signalSlices.length - SCREEN_WAVEFORM_CAP} lower-magnitude events not
                            shown — all {signalSlices.length} appear in the printed report appendix
                            (top 20 by magnitude).
                          </p>
                        )}
                      </div>
            ...
            <div className="card card-muted p-6 text-center">
              <p className="text-sm text-slate-500">
                No findings yet. Run analysis to extract structured findings.
              </p>
            </div>
          )}
        </div>
      ...
      {activeTab === 'plan' && (
        <div className="space-y-4">
          ...
          {!c.findings.length || !c.structuredReport ? (
            <div className="card card-muted p-6 text-center">
              <p className="text-sm text-slate-500">
                Run analysis first to enable action plan generation.
              </p>
            </div>
          ) : !reviewComplete && !c.actionPlan ? (
            <div className="card card-muted p-6 text-center">
              <p className="text-sm text-slate-500">
                Review every finding and populated report section before generating an action plan.
              </p>
            </div>
            ...
            <div className="card card-muted p-6 text-center space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
                  Synthesises high-confidence findings into priority actions, verification steps,
                  artefact caveats, and clinical context - tailored for the reviewing clinician.
                </p>
              </div>
              <button className="btn-teal mx-auto" onClick={() => void startActionPlan()}>
      ...
      {(locked || !!c.actionPlan) && c.findings.length > 0 && (
        <PrintableReport c={c} signalSlices={signalSlices} />
      )}
    </div>
  );
}
