...
import { Check, X, HelpCircle, Pencil, Info, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
...
const RESPIRATORY_TYPES = new Set(['apnea', 'hypopnea', 'obstructive_apnea', 'central_apnea', 'mixed_apnea']);
...
function isEdfBacked(report: StructuredReport, findings: Finding[], sectionKey: ReportSectionKey, fieldSource: string): boolean {
    ...
    >
      ...
      {hasArtifact && (
        <div className="px-3 py-2 border-b border-ui-border/60 flex items-start gap-1.5 text-amber-700 dark:text-amber-400">
          ...
          <span>Airflow channel {flatPct}% flat-segment artifact - may contain false-positive events</span>
        </div>
      )}

      <div className="px-3 py-2 border-b border-ui-border/60 space-y-1">
        <p className="text-ui-text-subtle uppercase tracking-wide text-[10px] font-medium mb-1.5">Indices (recording-time, not sleep-time)</p>
        <CalcRow label="Raw REI" value={rawRei != null ? `${rawRei}/h` : '-'} detail={d ? `${d.flowEventCount} events ÷ ${hours?.toFixed(2)}h` : undefined} />
        {adjRei !== rawRei && (
          <CalcRow
            label="Artifact-adjusted REI"
            value={adjRei != null ? `${adjRei}/h` : '-'}
            detail={d ? `${d.artifactAdjustedCount} events (excl. ${d.artifactExcludedCount} in flat periods) ÷ ${hours?.toFixed(2)}h` : undefined}
          />
        )}
        <CalcRow label="ODI 3%" value={odi != null ? `${odi}/h` : '-'} detail={edfMetrics.spo2?.desatCount != null ? `${edfMetrics.spo2.desatCount} desat events ÷ ${hours?.toFixed(2)}h` : undefined} />
      </div>

      {fs && (
        <div className="px-3 py-2 border-b border-ui-border/60 space-y-1">
          <p className="text-ui-text-subtle uppercase tracking-wide text-[10px] font-medium mb-1.5">Flow event breakdown</p>
          ...
          {fs.avgDurationSec != null && <CalcRow label="Avg duration" value={`${fs.avgDurationSec}s`} />}
          {fs.maxDurationSec != null && <CalcRow label="Max duration" value={`${fs.maxDurationSec}s`} />}
          {fs.severityBreakdown && Object.keys(fs.severityBreakdown).length > 0 && (
            <div className="mt-1">
              {Object.entries(fs.severityBreakdown).map(([key, n]) => (
                <CalcRow key={key} label={key.replace('provisional_flow_reduction_', '')} value={String(n)} />
              ))}
            </div>
          )}
        </div>
...
function CalcRow({ label, value, detail }: { label: string; value: string; detail?: string | undefined }) {
  return (
    <div className="flex items-baseline justify-between gap-2">
      <span className="text-slate-500 dark:text-slate-400 shrink-0">{label}</span>
      <span className="font-medium tabular-nums text-slate-800 dark:text-slate-200 text-right">
        {value}
        {detail && <span className="ml-1 font-normal text-[10px] text-slate-400 dark:text-slate-500">({detail})</span>}
      </span>
    </div>
  );
}
...
function renderSectionBody(
  ...
  switch (key) {
    case 'summary':
    case 'impression':
      return <p className="text-sm leading-relaxed whitespace-pre-wrap">{stripInlineCitations(report[key] as string)}</p>;
    ...
    case 'respiratoryIndices': {
      ...
      const ahiProvisional = r.ahi !== undefined && isEdfBacked(report, findings, 'respiratoryIndices', 'study_metrics.provisional_rei_per_hour');
      const odi3Provisional = r.odi3 !== undefined && isEdfBacked(report, findings, 'respiratoryIndices', 'study_metrics.provisional_odi_per_hour');

      const tstH = pdfMetrics?.total_sleep_time_seconds != null ? pdfMetrics.total_sleep_time_seconds / 3600 : null;
      const trtH = pdfMetrics?.total_recording_seconds != null ? pdfMetrics.total_recording_seconds / 3600 : (edfMetrics?.totalRecordingHours ?? null);
      ...
      return (
        <div className="text-sm grid grid-cols-2 gap-x-4 gap-y-1">
          <Row
            ...
            missingTitle={noPdf ? 'No DOMINO PDF - provisional REI from EDF' : 'AHI not found in DOMINO PDF'}
            calc={calc(r.ahi)}
          />
          <Row label="REI (artifact-adj)" value={r.reiArtifactAdjusted !== undefined ? fmt(r.reiArtifactAdjusted, '/h') : null} calc={calc(r.reiArtifactAdjusted)} provisional={r.reiArtifactAdjusted !== undefined} />
          <Row label="ODI 3%" value={r.odi3 !== undefined ? fmt(r.odi3, '/h') : null} provisional={odi3Provisional} calc={calc(r.odi3)} />
          <Row label="ODI 4%" value={r.odi4 !== undefined ? fmt(r.odi4, '/h') : null} calc={calc(r.odi4)} />
          <Row label="REI (DOMINO)" value={r.rei !== undefined ? fmt(r.rei, '/h') : null} calc={calc(r.rei)} />
          <Row label="Central index" value={r.centralIndex !== undefined ? fmt(r.centralIndex, '/h') : null} calc={calc(r.centralIndex)} />
          <Row label="Apnea count (prov.)" value={fmt(r.apneaCount)} />
          <Row label="Hypopnea count (prov.)" value={fmt(r.hypopneaCount)} />
          <Row label="Avg event duration" value={r.avgEventDurationSec !== undefined ? fmt(r.avgEventDurationSec, 's') : null} />
          <Row label="Max event duration" value={r.maxEventDurationSec !== undefined ? fmt(r.maxEventDurationSec, 's') : null} />
        </div>
      );
    }
    ...
    case 'oxygenation': {
      ...
      return (
        <div className="text-sm grid grid-cols-2 gap-x-4 gap-y-1">
          ...
          <Row label="Avg desat duration" value={o.avgDesatDuration !== undefined ? fmt(o.avgDesatDuration, 's') : null} />
          <Row label="Longest desat" value={o.longestDesatSec !== undefined ? fmt(o.longestDesatSec, 's') : null} />
          <Row label="Total desat time" value={o.sumDesatSec !== undefined ? fmt(Math.round(o.sumDesatSec / 60), ' min') : null} />
        </div>
      );
    }
    ...
    case 'positional': {
      ...
      const nonSupineTimePct =
        p.supineTimePct != null
          ? [p.leftTimePct, p.rightTimePct, p.proneTimePct, p.uprightTimePct]
              .reduce<number>((acc, v) => acc + (v ?? 0), 0) || (100 - p.supineTimePct)
          : undefined;
      return (
        <div className="text-sm grid grid-cols-2 gap-x-4 gap-y-1">
    ...
    case 'snoring': {
      ...
      return (
        <div className="text-sm grid grid-cols-2 gap-x-4 gap-y-1">
          {s.snoreIndex !== undefined && <Row label="Snore index" value={fmt(s.snoreIndex, '/h')} />}
          {s.snoreMinutes !== undefined && <Row label="Snore time" value={fmt(s.snoreMinutes, ' min')} />}
          {s.snoreTimePct !== undefined && <Row label="Snore time %" value={fmt(s.snoreTimePct, '%')} />}
        </div>
      );
    }
    ...
    case 'cardiac': {
      ...
      return (
        <div className="text-sm grid grid-cols-2 gap-x-4 gap-y-1">
          ...
          {c.wakeMeanHr !== undefined && <Row label="Mean HR (wake)" value={fmt(c.wakeMeanHr, ' bpm')} />}
          {c.wakeMinHr !== undefined && <Row label="Min HR (wake)" value={fmt(c.wakeMinHr, ' bpm')} />}
          {c.wakeMaxHr !== undefined && <Row label="Max HR (wake)" value={fmt(c.wakeMaxHr, ' bpm')} />}
        </div>
      );
    }
  }
...
function Row({ label, value, provisional, missingTitle, calc }: {
  label: ReactNode;
  value: string | null;
  provisional?: boolean;
  missingTitle?: string;
  ...
  return (
    <div className="flex justify-between gap-2 items-baseline">
      <span className="text-slate-500 dark:text-slate-400 flex items-center gap-0.5 shrink-0">{label}</span>
      {value !== null ? (
        <span
        ...
        >
          {value}{provisional && <sup className="ml-0.5 text-[9px]">~</sup>}
          {calc && <span className="ml-1 text-[11px] font-normal text-slate-400 dark:text-slate-500">({calc})</span>}
        </span>
      ) : (
        <span
...
export function StructuredReportView({ report, findings, reviews, locked, pdfMetrics, edfMetrics, signalSlices, onSectionDecision }: Props) {
  const findingIndexById = new Map(findings.map((f, i) => [f.id, i]));
  const findingById = new Map(findings.map((f) => [f.id, f]));

  function sectionHasPdfCitation(cited: string[]): boolean {
    return cited.some((id) =>
      findingById.get(id)?.evidence.some((e) => e.type === 'pdf_metric')
    );
  }
...
interface SectionCardProps {
  ...
  onDecision: (section: ReportSectionKey, decision: ReviewerDecision, editedValue?: string) => Promise<void>;
  children: ReactNode;
}

function SectionCard({ sectionKey, label, review, citedIds, findingIndexById, dominoMatched, locked, waveformSlices, onDecision, children }: SectionCardProps) {
  ...
  async function handleDecision(decision: ReviewerDecision) {
    if (saving) return;
    setSaving(true);
    try { await onDecision(sectionKey, decision); }
    ...
    <div className="card p-4 space-y-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 flex-wrap justify-end">
          {review && (
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400 capitalize">{review.decision}</span>
          )}
        </div>
      </div>
      ...
      {citedIds.some((id) => findingIndexById.has(id)) && (
        <div className="flex items-center gap-1.5 pt-2 border-t border-slate-100 dark:border-slate-800/60 flex-wrap">
          <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wide">Sources</span>
          {citedIds.map((id) => {
          ...
          {editMode ? (
            <>
              ...
              <button
                ...
                onClick={() => { setEditMode(false); setEditText(review?.editedValue ?? ''); }}
                disabled={saving}
              >Cancel</button>
            </>
          ) : (
            <>
              ...
              <button
                ...
                disabled={saving}
              ><HelpCircle size={12} /> Uncertain</button>
              <button
                ...
                onClick={() => { setEditText(review?.editedValue ?? ''); setEditMode(true); }}
                disabled={saving}
