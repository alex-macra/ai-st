  ...
  return (
    <ul className="space-y-1.5">
      {evidence.map((ev, i) => {
        ...
        const matchedSlice =
          ev.type === 'event_table' && ev.eventId
            ? signalSlices.find((s) => s.eventId === ev.eventId)
            : undefined;
        return (
          <li
            key={i}
            className="rounded-md bg-slate-50 dark:bg-slate-800/50 text-xs overflow-hidden"
          >
            <div className="flex items-start gap-2 p-2">
              <span
                className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium ${typeColor}`}
              >
                {typeText}
              </span>
              <div className="min-w-0 flex-1">
                ...
                {ev.timestamp && <span className="text-slate-400 ml-2">{ev.timestamp}</span>}
              </div>
            </div>
            {matchedSlice && (
