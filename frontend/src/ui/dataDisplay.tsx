...
export function QuotaCard({
  ...
  const selectedTone = tone ?? (percent >= thresholds.danger * 100 ? 'danger' : percent >= thresholds.warning * 100 ? 'warning' : progressToneFromPercent(percent));
  const remaining = unlimited ? null : Math.max(0, budget - used);
  const unitLabel = remaining === 1 ? unit : `${unit}s`;

  return (
    <section className={cx('rounded-2xl border border-ui-border bg-ui-bg-raised p-5', className)} aria-label={label}>
      <div className="mb-3 flex items-baseline justify-between gap-3">
        <h2 className="text-sm font-semibold text-ui-text">{label}</h2>
        <span className="text-xs tabular-nums text-ui-text-subtle">{used.toLocaleString()} / {unlimited ? 'Unlimited' : budget.toLocaleString()}</span>
      </div>
      ...
      <div className="mt-3 flex items-start justify-between gap-3 text-xs text-ui-text-muted">
        <span>{remainingLabel ?? (unlimited ? 'Unlimited' : `${remaining?.toLocaleString()} ${unitLabel.trim()} remaining`)}</span>
        {windowEndsAt !== undefined && <span>resets in {countdown(windowEndsAt - Date.now())}</span>}
      </div>
...
export function TokenUsagePanel({ rows, costUsd, model, title = 'Token usage', className }: TokenUsagePanelProps) {
  if (rows.length === 0) return null;
  const totalInput = rows.reduce((total, row) => total + row.input, 0);
  const totalOutput = rows.reduce((total, row) => total + row.output, 0);
  return (
    <section className={cx('space-y-2 rounded-lg border border-ui-border bg-ui-bg-raised p-3', className)}>
      ...
      <dl className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
        {rows.map((row) => (
          <div key={row.label}>
            ...
            <dd className="text-ui-text"><span aria-label="input tokens">↑ {row.input.toLocaleString()}</span> / <span aria-label="output tokens">↓ {row.output.toLocaleString()}</span></dd>
          </div>
        ))}
      </dl>
      <div className="flex justify-between border-t border-ui-border pt-2 text-xs text-ui-text-muted">
        <span>Total: ↑ {totalInput.toLocaleString()} / ↓ {totalOutput.toLocaleString()}</span>
...
export function StatCard({ label, value, delta, sub, icon, sparkline, loading = false, className }: StatCardProps) {
  return (
    <section className={cx('rounded-2xl border border-ui-border bg-ui-bg-raised p-5', className)}>
      <div className="flex items-start gap-3">
        {icon && <div aria-hidden="true" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-ui-accent/10 text-ui-accent">{icon}</div>}
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium text-ui-text-muted">{label}</p>
          {loading ? <div className="mt-2 h-7 w-20 animate-pulse rounded bg-ui-bg-muted" /> : <p className="mt-1 text-2xl font-semibold text-ui-text">{value}</p>}
          {delta && <p className={cx('mt-1 text-xs', delta.dir === 'up' ? 'text-green-600' : delta.dir === 'down' ? 'text-red-600' : 'text-ui-text-muted')}>{delta.value}{delta.label ? ` ${delta.label}` : ''}</p>}
